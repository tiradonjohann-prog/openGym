...
export default function Settings() {
  ...
  return <div className="narrow">
    ...
    <Section title={t("During a workout")} footer={wakeOK ? t("The screen stays on while a workout is running, so you don't have to unlock your phone between sets.") : null}>
      <Row icon="scalemass" iconTint="var(--blue)" title={t("Ask weight before workout")}
        subtitle={t("Log your body weight before every training session.")}>
        <Switch checked={S.promptWeighBefore !== false} onChange={v => update(s => { s.promptWeighBefore = v })} />
      </Row>
    ...
    <Section title={t("Experience")} footer={S.simpleMode ? t("Beginner mode on — progression settings and effort tracking are hidden. Toggle off to access all features.") : t("Turn on beginner mode to simplify the interface and focus on the essentials.")}>
      <Row icon="sparkles" iconTint="var(--teal)" title={t("Beginner mode")}
        subtitle={t("Hides advanced options — progression policy, effort per set.")}>
    ...
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      ...
      <a href="https://github.com/DuarteSantos8/openGym" target="_blank" rel="noopener noreferrer">source code</a> · exercise data: hasaneyldrm/exercises-dataset (CC)
    </div>
  </div>
}
